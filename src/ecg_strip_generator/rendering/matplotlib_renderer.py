...
from ecg_strip_generator.validation import PreparedSignal, prepare_signal
...
def build_figure(request: RenderRequest, signal: PreparedSignal, geometry: Geometry) -> Figure:
    ...
    for index, lead in enumerate(signal.leads):
        x, y, width, height = geometry.panel_bounds_mm(index)
        ax = fig.add_axes(
            (
        ...
        for step, colour, thickness in ((1, "#f4dada", 0.22), (5, "#dcaaaa", 0.45)):
            ...
            lines += [((0, v), (width, v)) for v in np.arange(0, height + 1e-9, step)]
            ax.add_collection(LineCollection(lines, colors=colour, linewidths=thickness, zorder=0))
        baseline = height / 2
        # A true 0.2-second by 1-mV calibration pulse; gutter never replaces samples.
        pulse_width = 0.2 * preset.paper_speed_mm_s
        ax.plot(
            [2, 3, 3, 3 + pulse_width, 3 + pulse_width, 4 + pulse_width],
            [
                baseline,
                baseline,
        ...
        ax.plot(
            geometry.calibration_gutter_mm + signal.times_s * preset.paper_speed_mm_s,
            baseline + signal.values_mv[:, index] * preset.gain_mm_mv,
            color="#202020",
            linewidth=0.7,
            solid_capstyle="butt",
            gid="waveform",
        )
        ax.text(1.5, height - 1.5, lead, va="top", fontsize=7, fontweight="bold")
    ...
    fig.text(
        ...
        f"Simultaneous | {start:g}-{start + signal.duration_s:g} s (end exclusive)",
        fontsize=6,
        va="top",
    )
    fig.text(
