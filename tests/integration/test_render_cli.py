...
def test_fresh_process_outputs_match(tmp_path) -> None:
    source = tmp_path / "request.json"
    source.write_bytes(canonical_json(make_fixture(2).model_dump(mode="json")))
    command = (
    ...
    assert manifest["preset"]["time_alignment"] == "simultaneous"
